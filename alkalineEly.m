function [Tely,eta_tot,V_H2,V_O2,Ustack] =  alkalineEly(ely,Ptot,Tini,on_off,Tambient,Tcool_in,Vcool_in)
...
Pely =Ptot/Ncells/Nstack;  % Power of each cell
...
if on_off == 0 || Prely<1e-6
    [Tely,V_H2,V_O2,eta_e,eta_f,Qloss] = ElyOff_c(therm_par,Ta,Ncells,Tini,Timestep);
...
elseif on_off == 1
    ...
    while (ii<100) &&(diff>0.001)
        ...
        [Utn,Urev] = ElyGibbs(Tely_avg, Prely);
        
        %
        [Ucell,Idensity,Iely] = ElyElec(IVcurve_par,Area,Pely,Tely_avg,Urev);
        
        %
        eta_f = ElyFarad(Farad_par,Idensity);
        
        %
        [Uely,Ptot,V_H2,V_O2,eta_tot,eta_e] = ElyStack(Ncells,Iely,eta_f,Ucell,Utn);
    ...
    function[Ucell, Idensity,Iely] =   ElyElec(Par, Area, Pely,Tely,Urev)
        r1 = Par(1);
        r2 = Par(2);
        s1 = Par(3);
        t1 = Par(4);
        ...
        % P = UI, and using fsolve to sole this equation
        Iely= fzero_ElyUI(Par,Area,Tely,Urev,Pely,[0,3500]);
%         Iely = fsolve(@(Iely)(((r1+r2*Tely)*Iely/Area + s1*log10((t1+t2/Tely+t3/(Tely^2))*Iely/Area+1)+Urev)*Iely - Pely), [1000],optimoptions('fsolve','Display','off'));
        Ucell = Pely/Iely;
        % current density
    ...
    function [Uely,Pstack,V_H2,V_O2,eta_tot,eta_e] = ElyStack( Ncells,Iely,eta_f,Ucell,Utn)
        FaradConst = 96485; %As/mol
        Rgas = 8.3145; %J/(k，mol��
        Nelec = 2;
        Tstd = 0; %centigrade
        ...
        n_O2 = 0.5* n_H2;
        % production flowrate
        V_H2 = n_H2 *(Rgas*(Tstd+273.15))/Pstd/100000*3600;  %Nm3/hr, Nm3--standard cubic meters
        V_O2 = n_O2 *(Rgas*(Tstd+273.15))/Pstd/100000*3600;  %Nm3/hr
